...
% ==========================================================
for i=1:4
    lon_bnd = squeeze(lon_bnds(i,:,:));
    for ilat=1:length(nlat)
        lon_temp = squeeze(lon_bnd(:,ilat));
        for j=2:length(nlon)-1
            if(abs(lon_temp(j)-lon_temp(j-1))>5.0 & abs(lon_temp(j+1)-lon_temp(j))>5.0)
                if(abs(lon_temp(j))<5.0)
                    lon_temp(j) = lon_temp(j)+180.0;
                else
                    lon_temp(j) = lon_temp(j)-180.0;
                end
            end
        end
        lon_bnd(:,ilat) = lon_temp;
    end
    lon_bnds(i,:,:) = lon_bnd;
end
% ==========================================================
...
figure;
m_proj('Azimuthal Equal-area','lon',180,'lat',90,'radius',40);
