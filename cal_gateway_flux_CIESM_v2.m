...
fname = 'areacello_Ofx_CIESM_historical_r1i1p1f1_gn.nc';
lat_bnds = ncread([dname,fname],'vertices_latitude');
lon_bnds = ncread([dname,fname],'vertices_longitude');
dir_root = '/media/swang/CMIP6_1/';
mdl_name = 'CIESM';
dir_mdl = [dir_root,mdl_name,'/'];

fsec = '~/Data/grid_and_mask/areacello_cmip6/CIESM_sections.mat';
load(fsec);
...
%             t_denmark = nansum(nansum(t2d.*dA))/nansum(nansum(dA(~isnan(s2d))));% net
            ...
            % In CIESM, it is BSO inflow when uo>0 or vo<0
            loc1u = find(u2d>0);
            loc2u = find(u2d<0);
            loc1v = find(v2d>0);
            loc2v = find(v2d<0);
