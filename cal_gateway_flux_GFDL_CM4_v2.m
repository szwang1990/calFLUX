...
fname = 'areacello_Ofx_GFDL-CM4_historical_r1i1p1f1_gn.nc';

lat_bnds = ncread([dname,fname],'lat_bnds');
lon_bnds = ncread([dname,fname],'lon_bnds');


dir_root = '/media/swang/CMIP6_4/';
mdl_name = 'GFDL-CM4';

dir_mdl = [dir_root,mdl_name,'/'];

fsec = '~/Data/grid_and_mask/areacello_cmip6/GFDL-CM4_sections.mat';
load(fsec);
...
for iensem=1:length(ensemble_list)
    for iscenario=1:length(scenario_list)
        ...
        for iyear=year_s:year_e
            if (iyear<=2014 & ~strcmp(char(scenario_list(iscenario)),'historical')) con tinue;end;
            ...
            for imon=1:12
                u3d_each = ncread(fu,'uo',[1 1 1 imon],[Inf Inf Inf 1],[1 1 1 1]);
                v3d_each = ncread(fv,'vo',[1 1 1 imon],[Inf Inf Inf 1],[1 1 1 1]);
                s3d_each = ncread(fs,'so',[1 1 1 imon],[Inf Inf Inf 1],[1 1 1 1]);
                t3d_each = ncread(ft,'thetao',[1 1 1 imon],[Inf Inf Inf 1],[1 1 1 1]);
                u3d = u3d + u3d_each;
                v3d = v3d + v3d_each;
                s3d = s3d + s3d_each;
                t3d = t3d + t3d_each;
...
%             t_denmark = nansum(nansum(t2d.*dA))/nansum(nansum(dA(~isnan(s2d))));% net
            ...
            vol_denmark(1,1) = nansum(nansum(v2d.*dA));% net
            vol_denmark(2,1) = nansum(nansum(v2d(loc1).*dA(loc1)));% in
            vol_denmark(3,1) = nansum(nansum(v2d(loc2).*dA(loc2)));% out
            fw_denmark(1,1)  = nansum(nansum(v2d.*dA.*(S_ref-s2d)/S_ref));%
            fw_denmark(2,1)  = nansum(nansum(v2d(loc1).*dA(loc1).*(S_ref-s2d(loc1))/S_ref));%
            fw_denmark(3,1)  = nansum(nansum(v2d(loc2).*dA(loc2).*(S_ref-s2d(loc2))/S_ref));%
            ht_denmark(1,1)  = 4.2e6*nansum(nansum(v2d.*dA.*(t2d-T_ref)));%
            ht_denmark(2,1)  = 4.2e6*nansum(nansum(v2d(loc1).*dA(loc1).*(t2d(loc1)-T_ref)));%
            ht_denmark(3,1)  = 4.2e6*nansum(nansum(v2d(loc2).*dA(loc2).*(t2d(loc2)-T_ref)));%
