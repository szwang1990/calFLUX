...
fname = 'areacello_Ofx_CNRM-CM6-1-HR_historical_r1i1p1f2_gn.nc';
lat_bnds = ncread([dname,fname],'bounds_lat');
lon_bnds = ncread([dname,fname],'bounds_lon');

dir_root = '/media/swang/CMIP6_2/';
mdl_name = 'CNRM-CM6-1-HR';
dir_mdl = [dir_root,mdl_name,'/'];

fsec = '~/Data/grid_and_mask/areacello_cmip6/CNRM-CM6-1-HR_sections.mat';
load(fsec);
...
%             t_denmark = nansum(nansum(t2d.*dA))/nansum(nansum(dA(~isnan(s2d))));% net
            ...
            fw_denmark(1,1)  = nansum(nansum(v2d.*dA.*(S_ref-s2d)/S_ref));% net
