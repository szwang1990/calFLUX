...
fname = 'areacello_Ofx_INM-CM5-0_historical_r1i1p1f1_gr1.nc';
...
fsec = '~/Data/grid_and_mask/areacello_cmip6/INM-CM5-0_sections.mat';
load(fsec);
...
%             t_davis = nansum(nansum(t2d.*dA))/nansum(nansum(dA(~isnan(s2d))));% net
            ...
            % In GISS, it is Denmark inflow when uo>0 or vo>0
            loc1u = find(u2d>0);
            loc2u = find(u2d<0);
            loc1v = find(v2d>0);
            loc2v = find(v2d<0);
...
%             s_bso = nansum(nansum(s2d.*dA))/nansum(nansum(dA(~isnan(s2d))));% net
%             t_bso = nansum(nansum(t2d.*dA))/nansum(nansum(dA(~isnan(s2d))));% net
            s_bso(1,1) = nansum(nansum(s2d.*dA))/nansum(nansum(dA(~isnan(s2d))));% net
            ...
            t_bso(1,1) = nansum(nansum(t2d.*dA))/nansum(nansum(dA(~isnan(s2d))));% net
            ...
            vol_bso(1,1) = nansum(nansum(u2d.*dA));% net
            vol_bso(2,1) = nansum(nansum(u2d(loc1).*dA(loc1)));% in
            vol_bso(3,1) = nansum(nansum(u2d(loc2).*dA(loc2)));% out
            fw_bso(1,1)  = nansum(nansum(u2d.*dA.*(S_ref-s2d)/S_ref));% net
            fw_bso(2,1)  = nansum(nansum(u2d(loc1).*dA(loc1).*(S_ref-s2d(loc1))/S_ref));% 
            fw_bso(3,1)  = nansum(nansum(u2d(loc2).*dA(loc2).*(S_ref-s2d(loc2))/S_ref));%
            ht_bso(1,1)  = 4.2e6*nansum(nansum(u2d.*dA.*(t2d-T_ref)));%
            ht_bso(2,1)  = 4.2e6*nansum(nansum(u2d(loc1).*dA(loc1).*(t2d(loc1)-T_ref)));%
            ht_bso(3,1)  = 4.2e6*nansum(nansum(u2d(loc2).*dA(loc2).*(t2d(loc2)-T_ref)));%
