...
fsec = '~/Data/grid_and_mask/areacello_cmip6/IPSL-CM6A-LR_sections.mat';
load(fsec);
...
for iensem=1:length(ensemble_list)
    for iscenario=1:length(scenario_list)
        ...
        for iyear=year_s:year_e
            ...
            dep = ncread(fu,'olevel');
            dep_bnds = ncread(fu,'olevel_bounds');
...
%             t_davis = nansum(nansum(t2d.*dA))/nansum(nansum(dA(~isnan(s2d))));% net
            ...
            % In IPSL, it is BSO inflow when uo>0 or vo>0
            loc1u = find(u2d>0);
            loc2u = find(u2d<0);
            loc1v = find(v2d>0);
            loc2v = find(v2d<0);
