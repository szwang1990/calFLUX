...
fname = 'areacello_Ofx_MCM-UA-1-0_historical_r1i1p1f1_gn.nc';

lon_mdl  = ncread([dname,fname],'longitude');
lat_mdl  = ncread([dname,fname],'latitude');
lat_bnds_orig = ncread([dname,fname],'lat_bnds');
lon_bnds_orig = ncread([dname,fname],'lon_bnds');
lat_bnds = zeros([4,size(lat_mdl)]);
lon_bnds = zeros([4,size(lat_mdl)]);
...
fsec = '~/Data/grid_and_mask/areacello_cmip6/MCM-UA-1-0_sections.mat';
load(fsec);
