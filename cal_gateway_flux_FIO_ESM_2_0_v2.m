...
dir_root = '/media/swang/CMIP6_3/';
mdl_name = 'FIO-ESM-2-0';
dir_mdl = [dir_root,mdl_name,'/'];

fsec = '~/Data/grid_and_mask/areacello_cmip6/FIO-ESM-2-0_sections_v3.mat';
% fu   = '/media/swang/CMIP6_0/NorESM2-LM/uo_Omon_NorESM2-LM_ssp126_r1i1p1f1_gr_201501-202012_2016.nc';
% fv   = '/media/swang/CMIP6_0/NorESM2-LM/vo_Omon_NorESM2-LM_ssp126_r1i1p1f1_gr_201501-202012_2015.nc';
% fs   = '/media/swang/CMIP6_4/NorESM2-LM/so_Omon_NorESM2-LM_ssp126_r1i1p1f1_gr_201501-202012_2015.nc';
% ft   = '/media/swang/CMIP6_4/NorESM2-LM/so_Omon_NorESM2-LM_ssp126_r1i1p1f1_gr_201501-202012_2015.nc';
% ncdisp(fu);
% ncdisp(fs);
load(fsec);
scenario_list = {'historical','ssp126','ssp245','ssp585'};
scenario_list = {'historical','ssp245','ssp585'};

ensemble_list = {'r1i1p1f1'};
year_s = 1950;
% year_e = 1948;
year_e = 2100;
...
%             t_fram = nansum(nansum(t2d.*dA))/nansum(nansum(dA(~isnan(s2d))));% net
            ...
            t_fram(2,1) = nansum(nansum(t2d(loc1).*dA(loc1)))/nansum(nansum(dA(loc1)));% in
            t_fram(3,1) = nansum(nansum(t2d(loc2).*dA(loc2)))/nansum(nansum(dA(loc2)));% out
            vol_fram(1,1) = nansum(nansum(u2d.*dA));% net
            vol_fram(2,1) = nansum(nansum(u2d(loc1).*dA(loc1)));% in
            vol_fram(3,1) = nansum(nansum(u2d(loc2).*dA(loc2)));% out
            fw_fram(1,1)  = nansum(nansum(u2d.*dA.*(S_ref-s2d)/S_ref));% net
            fw_fram(2,1)  = nansum(nansum(u2d(loc1).*dA(loc1).*(S_ref-s2d(loc1))/S_ref));% 
            fw_fram(3,1)  = nansum(nansum(u2d(loc2).*dA(loc2).*(S_ref-s2d(loc2))/S_ref));%
            ht_fram(1,1)  = 4.2e6*nansum(nansum(u2d.*dA.*(t2d-T_ref)));%
            ht_fram(2,1)  = 4.2e6*nansum(nansum(u2d(loc1).*dA(loc1).*(t2d(loc1)-T_ref)));%
            ht_fram(3,1)  = 4.2e6*nansum(nansum(u2d(loc2).*dA(loc2).*(t2d(loc2)-T_ref)));%
...
%             t_davis = nansum(nansum(t2d.*dA))/nansum(nansum(dA(~isnan(s2d))));% net
            ...
            loc1 = find(u2d>0);
            loc2 = find(u2d<0);
%             s_denmark(1,1) = nansum(nansum(s2d.*dA))/nansum(nansum(dA));% net
%             s_denmark(2,1) = nansum(nansum(s2d(loc1).*dA(loc1)))/nansum(nansum(dA(loc1)));% in
%             s_denmark(3,1) = nansum(nansum(s2d(loc2).*dA(loc2)))/nansum(nansum(dA(loc2)));% out
%             t_denmark(1,1) = nansum(nansum(t2d.*dA))/nansum(nansum(dA));% net
%             t_denmark(2,1) = nansum(nansum(t2d(loc1).*dA(loc1)))/nansum(nansum(dA(loc1)));% in
%             t_denmark(3,1) = nansum(nansum(t2d(loc2).*dA(loc2)))/nansum(nansum(dA(loc2)));% out
%             s_denmark = nansum(nansum(s2d.*dA))/nansum(nansum(dA(~isnan(s2d))));% net
%             t_denmark = nansum(nansum(t2d.*dA))/nansum(nansum(dA(~isnan(s2d))));% net

            s_denmark(1,1) = nansum(nansum(s2d.*dA))/nansum(nansum(dA(~isnan(s2d))));% net
            s_denmark(2,1) = nansum(nansum(s2d(loc1).*dA(loc1)))/nansum(nansum(dA(loc1)));% in
            s_denmark(3,1) = nansum(nansum(s2d(loc2).*dA(loc2)))/nansum(nansum(dA(loc2)));% out
            t_denmark(1,1) = nansum(nansum(t2d.*dA))/nansum(nansum(dA(~isnan(s2d))));% net
            t_denmark(2,1) = nansum(nansum(t2d(loc1).*dA(loc1)))/nansum(nansum(dA(loc1)));% in
            t_denmark(3,1) = nansum(nansum(t2d(loc2).*dA(loc2)))/nansum(nansum(dA(loc2)));% out
            vol_denmark(1,1) = nansum(nansum(u2d.*dA));% net
            vol_denmark(2,1) = nansum(nansum(u2d(loc1).*dA(loc1)));% in
            vol_denmark(3,1) = nansum(nansum(u2d(loc2).*dA(loc2)));% out
            fw_denmark(1,1)  = nansum(nansum(u2d.*dA.*(S_ref-s2d)/S_ref));%
            fw_denmark(2,1)  = nansum(nansum(u2d(loc1).*dA(loc1).*(S_ref-s2d(loc1))/S_ref));%
            fw_denmark(3,1)  = nansum(nansum(u2d(loc2).*dA(loc2).*(S_ref-s2d(loc2))/S_ref));%
            ht_denmark(1,1)  = 4.2e6*nansum(nansum(u2d.*dA.*(t2d-T_ref)));%
            ht_denmark(2,1)  = 4.2e6*nansum(nansum(u2d(loc1).*dA(loc1).*(t2d(loc1)-T_ref)));%
            ht_denmark(3,1)  = 4.2e6*nansum(nansum(u2d(loc2).*dA(loc2).*(t2d(loc2)-T_ref)));%
...
%             t_bso = nansum(nansum(t2d.*(dAx+dAy)))/nansum(nansum(dAx(~isnan(s2d))+dAy(~isnan(s2d))));% net
            ...
            s_bso(2,1) = (nansum(nansum(s2d(loc1u).*dAy(loc1u)))+nansum(nansum(s2d(loc2v).*dAx(loc2v))))/(nansum(nansum(dAy(loc1u)))+nansum(nansum(dAx(loc2v))));% in
            s_bso(3,1) = (nansum(nansum(s2d(loc2u).*dAy(loc2u)))+nansum(nansum(s2d(loc1v).*dAx(loc1v))))/(nansum(nansum(dAy(loc2u)))+nansum(nansum(dAx(loc1v))));% out
            t_bso(1,1) = nansum(nansum(t2d.*(dAx+dAy)))/nansum(nansum(dAx(~isnan(s2d))+dAy(~isnan(s2d))));% net
            t_bso(2,1) = (nansum(nansum(t2d(loc1u).*dAy(loc1u)))+nansum(nansum(t2d(loc2v).*dAx(loc2v))))/(nansum(nansum(dAy(loc1u)))+nansum(nansum(dAx(loc2v))));% in
            t_bso(3,1) = (nansum(nansum(t2d(loc2u).*dAy(loc2u)))+nansum(nansum(t2d(loc1v).*dAx(loc1v))))/(nansum(nansum(dAy(loc2u)))+nansum(nansum(dAx(loc1v))));% in
            vol_bso(1,1) = nansum(nansum(u2d.*dAy-v2d.*dAx));
            vol_bso(2,1) = nansum(nansum(u2d(loc1u).*dAy(loc1u)))-nansum(nansum(v2d(loc2v).*dAx(loc2v)));
            vol_bso(3,1) = nansum(nansum(u2d(loc2u).*dAy(loc2u)))-nansum(nansum(v2d(loc1v).*dAx(loc1v)));
            fw_bso(1,1)  = nansum(nansum(u2d.*dAy.*(S_ref-s2d)/S_ref))-nansum(nansum(v2d.*dAx.*(S_ref-s2d)/S_ref));%
            fw_bso(2,1)  = nansum(nansum(u2d(loc1u).*dAy(loc1u).*(S_ref-s2d(loc1u))/S_ref))-nansum(nansum(v2d(loc2v).*dAx(loc2v).*(S_ref-s2d(loc2v))/S_ref));%
            fw_bso(3,1)  = nansum(nansum(u2d(loc2u).*dAy(loc2u).*(S_ref-s2d(loc2u))/S_ref))-nansum(nansum(v2d(loc1v).*dAx(loc1v).*(S_ref-s2d(loc1v))/S_ref));%
            ht_bso(1,1)  = 4.2e6*nansum(nansum(u2d.*dAy.*(t2d-T_ref)))-nansum(nansum(v2d.*dAx.*(t2d-T_ref)));%
            ht_bso(2,1)  = 4.2e6*nansum(nansum(u2d(loc1u).*dAy(loc1u).*(t2d(loc1u)-T_ref)))-nansum(nansum(v2d(loc2v).*dAx(loc2v).*(t2d(loc2v)-T_ref)));%
            ht_bso(3,1)  = 4.2e6*nansum(nansum(u2d(loc2u).*dAy(loc2u).*(t2d(loc2u)-T_ref)))-nansum(nansum(v2d(loc1v).*dAx(loc1v).*(t2d(loc1v)-T_ref)));%
            
            % voltransp_bso = nansum(nansum(u2d.*dAy-v2d.*dAx));%
            
            fout = [dir_mdl,'/Gateway_v3_flux_yearly_',char(scenario_list(iscenario)),'_',char(ensemble_list(iensem)),'_',num2str(iyear)];
